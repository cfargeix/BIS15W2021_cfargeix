...
homerange %>% 
  ...
  geom_histogram()+
  labs(title = "Distribution of Acreage")
```
...
homerange %>% 
  select(log10.hra) %>% 
  summarise(min=min(log10.hra),
            median=median(log10.hra),
            max=max(log10.hra))
```

```{r}
homerange <- homerange %>% 
  mutate(range_category = case_when(log10.hra <= 0 ~ "very small",
                                    log10.hra > 0 & log10.hra <= 2.25 ~ "small",
                                    log10.hra > 2.25 & log10.hra <= 7 ~ "medium",
                                    log10.hra > 7 ~ "large"))
```
...
homerange %>% 
  ...
  ggplot(aes(x = class, fill = log10.mass)) +
  geom_bar() +
  coord_flip() +
  labs(title = "Mass by taxonomic class of small animals")
```
